...
fmt:
> $(PYTHON) -m black aics_bead_alignment_core
.PHONY: fmt

import-sort:
> $(PYTHON) -m isort aics_bead_alignment_core
.PHONY: import-sort

test:
> $(PYTHON) -m pytest aics_bead_alignment_core/tests/
.PHONY: test
...
docs:
> source $(ACTIVATE) && sphinx-apidoc -f -o docs aics_bead_alignment_core aics_bead_alignment_core/tests
